...
def string_to_visibility_level(level: str) -> VisibilityLevel:
    """Will convert a string to a visibility level. 将字符串转换为可见性级别。

    Note that it will quietly return ALL if
    the string is not matched to any visibility level string identifier.
    注意，如果字符串与任何可见性级别的字符串标识符不匹配，它将悄悄返回ALL。

    Args:
        level: visibility level as a string 作为字符串的可见性级别返回

    Returns:
        :class:`~py_trees.common.VisibilityLevel`: visibility level enum 可见性级别枚举
    """
    if level == "detail":
