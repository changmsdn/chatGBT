...
class Action(py_trees.behaviour.Behaviour):
    ...
    initiate a task_base and monitor the progress of that task_base at each tick
    until completed. While the task_base is running the behaviour returns
    :data:`~py_trees.common.Status.RUNNING`.

    On completion, the the behaviour returns with success or failure
    (depending on success or failure of the task_base itself).
    ...
    def __init__(self, name: str):
        ...
        最少的一次性初始化。一个很好的经验法则是只包括与能够将此行为插入到树中以离线渲染为点图相关的初始化。
        其他一次性初始化要求应通过setup（）方法满足。
        """
