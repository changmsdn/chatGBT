...
def get_level2_combine_prompt(bt_list, sent):
    prompt = f"""
...
def combine_level2_bt(bt_list, sent):
    ...
    filename = dir_root + "bt_composition_planner/level2_combine_rules.xml"
    ...
    # TODO: 2 所有规则都无法匹配，调用大模型，针对该句添加规则到配置文件 level1_combine_rules.xml 中
        ...
        for i in range(5):
            ...
            if not match:
                continue
            code = root.find("code")
            namespace = {'bt_list': bt_list, 'bt_create': create_bt_node}
