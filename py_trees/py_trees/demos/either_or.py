...
def description(root: py_trees.behaviour.Behaviour) -> str:
    ...
    content += "EVENTS\n"
    content += "\n"
    content += " -  3 : joystick one enabled, task_base one starts\n"
    content += " -  5 : task_base one finishes\n"
    content += " -  6 : joystick two enabled, task_base two starts\n"
    content += " -  7 : joystick one enabled, task_base one ignored, task_base two continues\n"
    content += " -  8 : task_base two finishes\n"
    content += "\n"
