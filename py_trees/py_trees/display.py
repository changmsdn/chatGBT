...
def _generate_text_tree(
    ...
    Args:
        root: the root of the tree, or subtree you want to show 要显示的树根或子树
        show_only_visited: show only visited behaviours 仅显示访问行为
        show_status: always show status and feedback message (i.e. for every element, not just those visited)
                    始终显示状态和反馈信息（即每个元素，而不仅仅是访问的元素）
        visited: dictionary of (uuid.UUID) and status (:class:`~py_trees.common.Status`)
            pairs for behaviours visited on the current tick 当前时间访问的行为配对
        previously_visited: dictionary of behaviour id/status pairs from the previous tree tick
                            上一个树标记中的行为id\/状态对字典
        indent: the number of characters to indent the tree 要缩进树的字符数
        symbols: dictates formatting style 指定格式样式
    ...
    Returns:
        a text-based representation of the behaviour tree 基于文本的行为树表示
